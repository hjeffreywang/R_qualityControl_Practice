...
Chrome <- read.csv("Furnace.txt", sep="")
#Chrome <- read.csv("Chrome.dat", sep="")
View(Chrome)
names(Chrome)[1]="Concentration"
Chrome
...
chrome.out$chart2.control.limits.lcl
...
spc.capability.cp.simple(lower.specification = 1.24, upper.specification = 1.31, process.natural.tolerance = *sigma.cc)

# Using sigma estimate of overall standard deviation (Pp)
spc.capability.cp.simple(lower.specification = 1.24, upper.specification = 1.31, process.natural.tolerance = 6*sd(Chrome$Concentration))
min(Chrome$Concentration)
max(Chrome$Concentration)
